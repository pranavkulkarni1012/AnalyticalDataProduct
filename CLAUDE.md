...
**Key architectural principle:** All transformation logic (CTEs, joins, aggregations) lives in SQL inside the config file. Pipeline code is generic and shared -- it contains NO transformation logic. Multiple data products share the same pipeline code; only the config differs.

**Orchestration:** The `/run-sdlc` skill is the single entry point for the full end-to-end pipeline. It generates a `run_id`, spawns each of the 6 subagents in sequence (Requirement Parser, Spec Generator, Config Generator, Pipeline Generator, Infra Agent, QA Agent), passes artifacts between stages, and cleans up intermediate files. All subagents use `model: claude-opus-4-6`.
...
### Terraform
- **MUST** use modules for all resource provisioning (generated by `/generate-terraform` skill).
...
This is a **minimal, agent-driven** project. All code generation logic lives in skills and agents
under `.claude/`. No pre-existing template files, scripts, or boilerplate -- everything is
generated on demand by the skills and agents.

```
AnalyticalDataProduct/
  .claude/
    agents/                   # 6 subagents (requirement-parser, spec-generator, etc.)
    skills/                   # 10 skills (generate-pipeline, validate-config, etc.)
    settings.json             # Hooks and tool permissions
    settings.local.json       # Local overrides (MCP permissions, allowed commands)
  CLAUDE.md                   # This file -- project instructions and hard constraints
  instruction.md              # Producer guide for using the AI SDLC framework
  requirements.txt            # Python dependencies
  README.md                   # Project overview
```

All knowledge -- config schema, code patterns, terraform modules, reconciliation logic,
CI/CD definitions -- lives inside the skills and agents. No external template files,
scripts, or boilerplate folders.

### Generated at Runtime (not checked in)
When skills and agents run, they generate code into these directories:
```
  ...
  pipelines/
    ...
    {product}/                # Step Function ASL, tests (generated by pipeline-generator agent)
  terraform/
  ...
  artifacts/{run_id}/         # Per-run artifacts (requirements, spec, config, code, report)
```
...
Every data product is defined by a single YAML config file in `configs/`. The config schema
is embedded in the `/validate-config` skill. Required top-level sections:
