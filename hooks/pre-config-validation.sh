...
if [ -n "$FILE_PATH" ] && echo "$FILE_PATH" | grep -q '\.yaml$'; then
  ...
  if [ -n "$CONTENT" ]; then
    if echo "$CONTENT" | grep -qiE "(INSERT INTO|MERGE INTO|UPDATE |DELETE FROM|CREATE TABLE|DROP TABLE|ALTER TABLE|TRUNCATE)" 2>/dev/null; then
      echo "BLOCKED: Config contains Snowflake write operations. Snowflake is read-only." >&2
      echo "Snowflake sources must use SELECT only. Remove any INSERT, MERGE, UPDATE, DELETE, CREATE TABLE, DROP TABLE, ALTER TABLE, or TRUNCATE statements." >&2
  ...
  if [ -f "$FILE_PATH" ]; then
    if grep -qiE "(INSERT INTO|MERGE INTO|UPDATE |DELETE FROM|CREATE TABLE|DROP TABLE|ALTER TABLE|TRUNCATE)" "$FILE_PATH" 2>/dev/null; then
      echo "BLOCKED: Config contains Snowflake write operations. Snowflake is read-only." >&2
      echo "Snowflake sources must use SELECT only. Remove any INSERT, MERGE, UPDATE, DELETE, CREATE TABLE, DROP TABLE, ALTER TABLE, or TRUNCATE statements." >&2
      exit 2  # Exit 2 = block the operation (Claude Code hook standard)
    fi
